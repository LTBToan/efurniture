...
app.post("/users", async (req, res) => {
  const sql =
    "INSERT INTO users (user_id, email, password, fullName, role_id, phone, create_at, status) VALUES (?,?,?,?,?,?,?,?)";
  const values = [
    req.body.user_id,
    req.body.email,
    req.body.password,
...
app.get("/topProducts", (req, res) => {
  const sql =
    "SELECT * FROM products WHERE status = 1 ORDER BY RAND() LIMIT 20";
  db.query(sql, (err, result) => {
    if (err) console.log(err.message);
    return res.json(result);
  });
...
app.get("/productsOfTheWeek", (req, res) => {
  const sql = "SELECT * FROM products WHERE status = 1 ORDER BY RAND() LIMIT 4";
  db.query(sql, (err, result) => {
    if (err) console.log(err.message);
    return res.json(result);
  });
...
app.get("/orders/:orderId", (req, res) => {
  ...
  db.query(sql, [orderId], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Error met while fetching order" });
    }
    if (result.length === 0) {
...
app.get("/orders/user/:userId", (req, res) => {
  const userId = req.params.userId;
  const sql =
    "SELECT * FROM orders WHERE user_id = ? ORDER BY status ASC, date DESC";
  db.query(sql, [userId], (err, result) => {
    if (err) {
      console.error(err);
      return;
...
app.get("/bookings", (req, res) => {
  const sql =
    "SELECT b.booking_id, b.date, b.time, b.status, b.contents, u.fullName AS fullName, p.name AS productName FROM bookings b JOIN users u ON b.user_id = u.user_id JOIN products p ON b.product_id = p.product_id";
  db.query(sql, (err, result) => {
    if (err) {
      console.log(err);
      return;
...
app.get("/feedbacks", (req, res) => {
  const sql =
    "SELECT f.feedback_id, f.createdAt, f.description, u.fullName AS fullName, p.product_id AS productId, p.name AS productName, p.image_url AS productImage FROM feedbacks f JOIN users u ON f.user_id = u.user_id JOIN products p ON f.product_id = p.product_id";
  db.query(sql, (err, result) => {
    if (err) {
      console.log(err);
      return;
...
app.get("/feedbacks/product/:id", (req, res) => {
  const productId = req.params.id;
  const sql =
    "SELECT f.feedback_id, f.createdAt, f.description, u.fullName AS fullName FROM feedbacks f JOIN users u ON f.user_id = u.user_id WHERE f.product_id = ?";
  db.query(sql, [productId], (err, result) => {
    if (err) {
      console.log(err);
      return;
